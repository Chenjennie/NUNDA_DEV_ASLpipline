function out = spm_dicom_convertP50(hdr,opts,root_dir,format)
...
%CYF added for FM KIRBY Hack
% for n=1:length(hdr)
%     for b=1:length(hdr{n})
%         test((n-1)*25+b,:)={hdr{1,n}{1,b}.TemporalPositionIdentifier,...
%             hdr{1, n}{1, b}.NumberofSlicesMR,hdr{1, n}{1, b}.Private_2005_1429};end
% end
fnames = cell(length(hdr),1);
for i=1:length(hdr),
...
if checkfields(hdr{1},'AcquisitionTime','MagneticFieldStrength','MRAcquisitionType',...
    ...
    %CYF KIRBY Hack
elseif checkfields(hdr{1},'TemporalPositionIdentifier','NumberofTemporalPositions',...
        'Private_2005_1429','MagneticFieldStrength','MRAcquisitionType',...
        'ScanningSequence','RepetitionTime','EchoTime','FlipAngle')
    descrip = sprintf('%gT %s %s TR=%gms/TE=%gms/FA=%gdeg Rep%g/%g %s',...
        hdr{1}.MagneticFieldStrength, hdr{1}.MRAcquisitionType,...
        deblank(hdr{1}.ScanningSequence),...
        hdr{1}.RepetitionTime,hdr{1}.EchoTime,hdr{1}.FlipAngle,...
        hdr{1}.TemporalPositionIdentifier,hdr{1}.NumberofTemporalPositions,...
        hdr{1}.Private_2005_1429);
    %CYF end of KIRBY Hack
else
...
if strcmp(root_dir, 'flat')
    ...
    %-------------------------------------------------------------------
    %CYF
    if ~isempty(strfind(hdr.Manufacturer,'Philips'))&&checkfields(hdr,'TemporalPositionIdentifier','Private_2005_1429')
        fname = sprintf('%s%s-%.4d-%.4d-%s.%s', prefix, strip_unwanted(hdr.PatientID),...
                    hdr.SeriesNumber, hdr.TemporalPositionIdentifier, deblank(hdr.Private_2005_1429),format);
    else
        if checkfields(hdr,'SeriesNumber','AcquisitionNumber')
            if checkfields(hdr,'EchoNumbers')
                fname = sprintf('%s%s-%.4d-%.4d-%.6d-%.2d.%s', prefix, strip_unwanted(hdr.PatientID),...
