...
%CYF
if isfield(flags,'ctrfirst') ctrfirst=flags.ctrfirst; 
else ctrfirst=0; end
...
if length(P)==1,
    ...
    %CYFif nargout==0, cleanandsave_parameters(P{1}); end;
    n = length(P{1});
    ...
    for s=1:numel(P),
        ...
        %CYFif nargout==0, cleanandsave_parameters(P{s}); end;
        n = length(P{s});
        ref=-ones(n,1);           %% ZW  should be switched later to allow different label control order, now assume first image is label
        ref(2:2:end)=1;
        if(ctrfirst) ref=-ref; end;
        P{s}=cleanandsave_parameters(P{s},ref);
    end;
end;
...
%_______________________________________________________________________
function Vo=cleanandsave_parameters(V,ref)
